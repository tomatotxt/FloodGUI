...
- `vendor/airflow.luau`: Airflow UI source supplied directly by the project owner. Its provenance and license were not supplied. It is included as an optional, vendored UI library and is not yet the application UI. FloodGUI changes use touch sizing on all touch-capable devices and associate slider/color-picker drags with their initiating pointer. It loads its supplied Lucide icon source and optional font files remotely when requested; failed downloads, compilation, or registration retain the built-in fallback symbols and BuilderSans font.
